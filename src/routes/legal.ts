...
router.get("/Privacy-Policy.html", (req: Request, res: Response) => {
  res.status(200).type("html").send(privacyHtml);
});

router.get("/Refund-Policy.html", (req: Request, res: Response) => {
  res.status(200).type("html").send(refundHtml);
});

router.get("/Terms-And-Conditions.html", (req: Request, res: Response) => {

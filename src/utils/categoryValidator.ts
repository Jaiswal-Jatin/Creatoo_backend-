...
export interface SalonAttributes {
  services?: Array<{ name: string; price: number; duration_minutes: number; description?: string; add_ons?: Array<{ name: string; price: number }> }>;
  stylists?: string[];
  gender_support?: 'men' | 'women' | 'unisex';
}
...
export function validateCategoryAttributes(
  category: 'restaurant' | 'salon' | 'turf' | null | string,
  attributes: any
): { status: boolean; message?: string; cleanData?: any } {
  if (!category) {
  ...
  if (category === 'restaurant') {
    ...
    if ('cuisine_type' in parsedAttrs && parsedAttrs.cuisine_type != null) {
      ...
      cleanData.cuisine_type = parsedAttrs.cuisine_type.map((c: any) => String(c).trim());
    }
      ...
      for (const service of parsedAttrs.services) {
        ...
        if (service.add_ons && Array.isArray(service.add_ons)) {
          cleanService.add_ons = service.add_ons
            .filter((a: any) => a && a.name && typeof a.name === 'string')
            .map((a: any) => ({
              name: a.name.trim(),
    ...
    if ('stylists' in parsedAttrs && parsedAttrs.stylists != null) {
      ...
      cleanData.stylists = parsedAttrs.stylists.map((s: any) => String(s).trim());
    }
    ...
    if ('sport_types' in parsedAttrs && parsedAttrs.sport_types != null) {
      ...
      cleanData.sport_types = parsedAttrs.sport_types.map((s: any) => String(s).trim());
    }
    ...
    if ('amenities' in parsedAttrs && parsedAttrs.amenities != null) {
      ...
      cleanData.amenities = parsedAttrs.amenities.map((a: any) => String(a).trim());
    }

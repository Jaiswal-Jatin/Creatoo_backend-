...
const BusinessController = {
  ...
  async customerSummary(req: Request, res: Response) {
    try {
      ...
      const cards = await Card.findAll({
        ...
        group: ["user_id"],
        raw: true, // <- important so we can use plain object props
      });

      const memberSinceMap = new Map<number, string | null>();

      cards.forEach((c: any) => {
        const uid = Number(c.user_id); // ensure numeric key
  ...
  // -----------------------------
  // POST /api/business/getBusinessList (public)
  // -----------------------------

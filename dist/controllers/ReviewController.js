...
const Order_1 = __importDefault(require("../models/Order")); // ✅ your Order model
...
class ReviewController {
    ...
    async reviewSubmit(req, res) {
        try {
            const { user_id, business_id, experience, expectation, recommend, fair_money, interaction, review_text, role_id, order_id, } = req.body;
            // ---------- Validation ----------
            const errors = {};
            if (!user_id)
            ...
            if (!expectation)
                errors.expectation = ["expectation is required"];
            if (!recommend)
                errors.recommend = ["recommend is required"];
            if (!fair_money)
                errors.fair_money = ["fair_money is required"];
            if (!interaction)
            ...
            const business = await Business_1.default.findByPk(businessIdNum) || await User_1.default.findByPk(businessIdNum);
            if (!business) {
                return res.status(422).json({
                    status: false,
            ...
            if (loyaltyPointsEarned > 0) {
                await CreatorPointsTransaction_1.default.create(transactionData);
            }
    ...
    async skipReview(req, res) {
        try {
            const { user_id, order_id, is_redeemed } = req.body;
            const errors = {};
            if (!user_id)
                errors.user_id = ["user_id is required"];
            if (!order_id)
            ...
            const notification_text = "You skipped the feedback survey. Complete it now to earn your reward Points!";
            const isRedeemedValue = is_redeemed ?? "0";
            const notificationRow = await NewUserNotification_1.default.create({
                user_id: userIdNum,
                order_id,
                notification_text,
            ...
            if (rememberToken) {
                const notificationPayload = {
                    ...
                    description: "You skipped the feedback survey. Complete it now to earn your reward Points!",
                };
            ...
            return res.status(500).json({
                status: false,
                message: "An error occurred while skipping review: " +
                    (e?.message || "Unknown error"),
            });
        }
    }

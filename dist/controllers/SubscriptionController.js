...
const service = new subscription_service_1.default();
/**
 * Helper: find invoice using Razorpay order id or fallback to transaction_id
 */
const findInvoiceByOrder = async (orderId) => {
    if (!orderId)
        return null;
    // 1) Try by razorpay_order_id
    let invoice = await invoice_1.Invoice.findOne({
        where: { razorpay_order_id: orderId },
    });
    // 2) Fallback by transaction_id
    if (!invoice) {
        invoice = await invoice_1.Invoice.findOne({
...
exports.default = {
    ...
    async addSubscription(req, res) {
        try {
            const { business_id, plan_id, payment_method, transaction_id, notes, status, } = req.body;
            if (!business_id) {
    ...
    // ===============================================================
    async purchaseForBusiness(req, res) {
        try {
            const user = req.user || {};
            ...
            if (Number.isNaN(amount) || amount <= 0) {
                console.error("❌ Invalid price returned from subscription for Razorpay:", amount);
                return res
                    .status(500)
            ...
            await Subscription_1.default.update({ transaction_id: order.id }, { where: { id: subscription.id } });
    ...
    async paymentSuccess(req, res) {
        try {
            ...
            if (!invoice) {
                console.error("❌ Invoice not found for Razorpay order id:", razorpay_order_id);
            ...
            if (invoice.subscription_id) {
                subscription = await Subscription_1.default.findByPk(invoice.subscription_id);
            }
            ...
            if (subscription) {
                ...
                console.warn("⚠️ Subscription not found for invoice, only invoice marked as paid");
            }
            return res.json({
                status: true,
    ...
    // ==============================
    async getEditSubscription(req, res) {
        try {
    ...
    async updateSubscription(req, res) {
        try {
            ...
            if (typeof req.body.auto_renew !== "undefined") {
    ...
    async changeStatus(req, res) {
        try {
            ...
            if (newStatus === "active") {
                const activeCount = await Subscription_1.default.count({
                    where: {
    ...
    // ==============================
    async getMyRemainingDays(req, res) {
        try {
            const user = req.user;
            const business_id = user?.id;
            if (!business_id) {
            ...
            const remaining_days = Math.max(0, Math.ceil(diffMs / (1000 * 60 * 60 * 24)));
            return res.json({
                remaining_days,

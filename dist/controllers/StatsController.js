...
const StatsController = {
    ...
    async overallStats(req, res) {
        try {
            ...
            const totalVisits = await Visit_1.default.count();
            const aggAll = await CreatorPointsTransaction_1.default.findOne({
                attributes: [
            ...
            const totalPoints = Number(aggAll?.total_points ?? 0);
            const totalSpent = Number(aggAll?.total_spent ?? 0);
            // --------- 2) User-wise stats ---------
            const visitRows = await Visit_1.default.findAll({
                where: { user_id: { [sequelize_1.Op.ne]: null } }, // per-user visits
            ...
            const cards = await Card_1.default.findAll({
                ...
                attributes: [
                    "user_id",
                    [(0, sequelize_1.fn)("MIN", (0, sequelize_1.col)("created_at")), "member_since"],
                ],
                group: ["user_id"],
                raw: true,
            });
            ...
            const usersData = visitRows.map((v) => {
            ...
            return res.status(500).json({
                status: false,
                message: "Failed to fetch admin overall stats: " +
                    (err.message || "Unknown"),
            });
        }
    },
    ...
    // -----------------------------
    async overallBusinessStats(req, res) {
        try {
            const adminId = req.user?.id;
            if (!adminId) {
                return res
            ...
            const totalBusinesses = await User_1.default.count();
            const totalVisits = await Visit_1.default.count();
            const aggAll = await CreatorPointsTransaction_1.default.findOne({
                attributes: [
                    [(0, sequelize_1.fn)("COALESCE", (0, sequelize_1.fn)("SUM", (0, sequelize_1.col)("points")), 0), "total_points"],
            ...
            const visitRows = await Visit_1.default.findAll({
                attributes: ["business_id", [(0, sequelize_1.fn)("COUNT", (0, sequelize_1.col)("id")), "total_visits"]],
                group: ["business_id"],
                raw: true,
            });
            ...
            const businessesData = visitRows
                .filter((v) => v.business_id !== null && v.business_id !== undefined)
                .map((v) => {
                const businessId = v.business_id;
                return {
                    business_id: businessId,
                    business_name: businessMap.get(businessId)?.business_name ?? "Unknown",
                    business_email: businessMap.get(businessId)?.business_email ?? null,
                    business_address: businessMap.get(businessId)?.business_address ?? null,
                    total_visits: Number(v.total_visits ?? v["total_visits"]) ?? 0,
            ...
            return res.status(500).json({
                status: false,
                message: "Failed to fetch admin overall business stats: " +
                    (err.message || "Unknown"),
            });
        }
    },
    ...
    // ----------------------------------------
    async discountStatsByUser(req, res) {
        try {
            const adminId = req.user?.id;
            if (!adminId) {
                return res
            ...
            const overallAgg = await Order_1.default.findOne({
                attributes: [
                    [
                        (0, sequelize_1.fn)("COALESCE", (0, sequelize_1.fn)("SUM", (0, sequelize_1.col)("original_bill_amount")), 0),
                        "total_original_bill",
                    ],
                    [
                        (0, sequelize_1.fn)("COALESCE", (0, sequelize_1.fn)("SUM", (0, sequelize_1.col)("discounted_bill")), 0),
                        "total_discounted_bill",
                    ],
                    [
                        (0, sequelize_1.literal)("COALESCE(SUM(original_bill_amount),0) - COALESCE(SUM(discounted_bill),0)"),
                        "total_discount_amount",
                    ],
                    [
                        (0, sequelize_1.fn)("COALESCE", (0, sequelize_1.fn)("AVG", (0, sequelize_1.col)("discount_percentage")), 0),
            ...
            const rows = await Order_1.default.findAll({
                attributes: [
                    "user_id",
                    [
                        (0, sequelize_1.fn)("COALESCE", (0, sequelize_1.fn)("SUM", (0, sequelize_1.col)("original_bill_amount")), 0),
            ...
            return res.json({
                ...
                data: {
                    overall: {
                        ...
                        total_discounted_bill: Number(overallAgg.total_discounted_bill ?? 0),
                        total_discount_amount: Number(overallAgg.total_discount_amount ?? 0),
                        avg_discount_percentage: Number(overallAgg.avg_discount_percentage ?? 0),
                    },
                    users: data,
                },
            });
            ...
            return res.status(500).json({
                status: false,
                message: "Failed to fetch discount stats by user: " +
                    (err.message || "Unknown"),
            });
        }
    },
    ...
    // ----------------------------------------
    async discountStatsByBusiness(req, res) {
        try {
            const adminId = req.user?.id;
            if (!adminId) {
                return res
            ...
            return res.status(500).json({
                status: false,
                message: "Failed to fetch discount stats by business: " +
                    (err.message || "Unknown"),
            });
        }
    },
    ...
    // ----------------------------------------
    async pointsStatsByUser(req, res) {
        try {
            const adminId = req.user?.id;
            if (!adminId) {
                return res
            ...
            // overall totals across all users
            const overallAgg = await CreatorPointsTransaction_1.default.findOne({
                attributes: [
                    // total earned (credit)
                    [
                        (0, sequelize_1.literal)("COALESCE(SUM(CASE WHEN LOWER(credit_debit_remaining_status) = 'credit' THEN points ELSE 0 END), 0)"),
                        "total_points_earned",
                    ],
                    // total redeemed (debit)
                    [
                        (0, sequelize_1.literal)("COALESCE(SUM(CASE WHEN LOWER(credit_debit_remaining_status) = 'debit' THEN points ELSE 0 END), 0)"),
                        "total_points_redeemed",
                    ],
                    // current balance (remaining_points for credit rows)
                    [
                        (0, sequelize_1.literal)("COALESCE(SUM(CASE WHEN LOWER(credit_debit_remaining_status) = 'credit' THEN remaining_points ELSE 0 END), 0)"),
            ...
            const rows = await CreatorPointsTransaction_1.default.findAll({
                attributes: [
                    "user_id",
                    [
                        (0, sequelize_1.literal)("COALESCE(SUM(CASE WHEN LOWER(credit_debit_remaining_status) = 'credit' THEN points ELSE 0 END), 0)"),
            ...
            return res.json({
                ...
                data: {
                    overall: {
                        ...
                        total_points_redeemed: Number(overallAgg.total_points_redeemed ?? 0),
                        total_points_balance: Number(overallAgg.total_points_balance ?? 0),
                    },
                    users,
                },
            });
            ...
            return res.status(500).json({
                status: false,
                message: "Failed to fetch points stats by user: " +
                    (err.message || "Unknown"),
            });
        }
    },
    ...
    // ----------------------------------------
    async pointsStatsByBusiness(req, res) {
        try {
            const adminId = req.user?.id;
            if (!adminId) {
                return res
            ...
            return res.status(500).json({
                status: false,
                message: "Failed to fetch points stats by business: " +
                    (err.message || "Unknown"),
            });
        }
    },
    ...
    async cardsStats(req, res) {
        try {
            ...
            // ---------- 1) Overall counts ----------
            const totalCards = await Card_1.default.count();
            const activeCards = await Card_1.default.count({ where: { status: 1 } });
            const inactiveCards = await Card_1.default.count({ where: { status: 0 } });
            // ---------- 2) Load all cards ----------
            const cards = await Card_1.default.findAll({
                attributes: [
                    "id",
            ...
            const userIds = Array.from(new Set(cards
                .map((c) => c.user_id != null ? Number(c.user_id) : null)
            ...
            const list = cards.map((c) => {
                const uid = c.user_id != null && !Number.isNaN(Number(c.user_id))
                    ? Number(c.user_id)
                    : null;
            ...
            return res.status(500).json({
                status: false,
                message: "Failed to fetch card stats: " + (err.message || "Unknown"),
            });
        }
    },
};

...
async function computeTotalAmount(budget, userId) {
    const user = await User_1.default.findByPk(userId);
    if (!user)
        throw new Error('User not found');
    const s = await Setting_1.default.findOne();
    if (!s)
        throw new Error('Settings not found');
    const gst = user.gst_number || '';
    const isMaharashtra = gst.endsWith('27');
    const platformFee = Number(s.platform_fee_percent || 0);
    let totalGst = 0;
    if (isMaharashtra) {
        const cgst = Number(s.cgst_percent || 0);
        const sgst = Number(s.sgst_percent || 0);
        totalGst = cgst + sgst;
    }
    else {
        totalGst = Number(s.igst_percent || 0);
    }
...
exports.default = {
    // GET /api/post/all
    async getAllPostService(req, res) {
        try {
            ...
            const users = await User_1.default.findAll({ where: { id: { [sequelize_1.Op.in]: userIds }, role_id: 2 } });
            const userMap = new Map(users.map(u => [u.id, u]));
            const filtered = records.filter((r) => userMap.has(r.user_id));
            const formatted = filtered.map((r) => {
                ...
                return {
                    ...
                    user_id: r.user_id,
                    user_name: u.name,
                    business_fullname: u.business_fullname,
    ...
    async addPost(req, res) {
        try {
            const { name, description, duration, deliverable, followers_required, work_mode, creator_required, per_creator_amount, post_status, post_expiry_date, } = req.body;
            ...
            if (String(post_status) === '1') {
                ...
                const tokensUsers = await User_1.default.findAll({
                    where: { remember_token: { [sequelize_1.Op.ne]: null } },
                ...
                const tokens = tokensUsers
                    ...
                    .filter(Boolean);
                if (tokens.length) {
                    await (0, sendPushNotification_1.sendPushNotification)({ title: 'NEW POST', description: `A new post has been added with post: ${name}` }, tokens);
                }
            }
    ...
    async updatePost(req, res) {
        try {
            const id = Number(req.params.id);
            const { name, description, duration, deliverable, followers_required, work_mode, creator_required, per_creator_amount, post_status, transaction_d, } = req.body;
            ...
            if (String(post.post_status) === '0' && String(post_status) === '1') {
                const tokensUsers = await User_1.default.findAll({
                    where: { remember_token: { [sequelize_1.Op.ne]: null } },
                    attributes: ['remember_token'],
                });
                const tokens = tokensUsers
                    ...
                    .filter(Boolean);
                if (tokens.length) {
                    await (0, sendPushNotification_1.sendPushNotification)({ title: 'NEW POST', description: `A new post has been added with post: ${post.name}` }, tokens);
                }
            }
    ...
    async changeStatus(req, res) {
        try {
            ...
            if (!id)
                return res.status(422).json({ status: false, message: 'id required' });
    ...
    async myPost(req, res) {
        try {
            ...
            const rows = await sequelize_2.default.query(`
    ...
    async postPaymentStatus(req, res) {
        try {
            const { user_id, post_id, payment_status, payment_status_response, } = req.body;
            ...
            if (!payment_status_response)
                missing.push("payment_status_response");
            if (missing.length) {
                return res.status(422).json({
                    status: false,
            ...
            // 🔎 ensure user exists
            const user = await User_1.default.findByPk(userIdNum);
            if (!user) {
    ...
    async getPostApplicationList(req, res) {
        try {
            ...
            const postInterests = await PostInterest_1.default.findAll({
            ...
            const creatorsWithFlags = creators.map((creator) => {
                const cJson = creator.toJSON();
                const pi = postInterests.find((p) => p.creator_id === creator.id);
                return {
                    ...cJson,
